...
function InfluencerCard({ item, onEdit, onDelete, onCopy }) {
  return (
...
function Drawer({ open, editItem, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      if (editItem) await updateInfluencerAPI(editItem._id, form);
      else await createInfluencerAPI(form);
      onSaved();
      onClose();
    } finally {
      setLoading(false);
...
export default function InfluencerList() {
  ...
  const handleSaved = () => {
    toast.success(editItem ? "Influencer updated" : "Influencer added");
    fetchData();
  };
  ...
  return (
    <div className="masterContainer">
      <div className={styles.container}>
        ...
        <div className={styles.header}>
          ...
          <div className={styles.actions}>
            ...
            <button onClick={handleAdd} className={styles.addButton}>
              <BiPlus style={{ fontSize: 16 }} /> Add influencer
            </button>
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className={styles.emptyState}>No influencers found.</div>
        ) : (
